...
**Tune LAN multiplayer**: `LAN_STATE_MS` / `LAN_PAD_MS` (snapshot/paddle rates), guest smoothing constants inside `lanApplyLatestState()` / `lanHostStep()`, room behavior + protocol in `lan-server.js` and app.js section 21, lobby copy in `lanSetupReadyCheck()` / `#lan-note`. Remember: the host is authoritative; never let the guest mutate shared match state directly.
