...
1. **Coordinate system**: world units are meters. The table is centered at the origin, top surface at `y = TABLE.H (0.76)`. **z: −far … +near**. P1's paddle lives at `z ≈ PADDLE_Z (1.30)`; in 2P/LAN, P2 takes the far rail at `PADDLE_Z_FAR (−1.15)`; the AI sits at `z ≈ −1.15`. The net is the plane `z = 0`, top at `NET_TOP (0.9125)`.
...
| `app.js` | Entire game (~3888 lines, one ES module file, banner-commented sections). |
| `lan-server.js` | Zero-dependency Node server for LAN 2P: serves the static files AND relays WebSocket messages between exactly two players (hand-rolled RFC 6455 — text frames, ping/pong, close, plus a 15s server→client heartbeat that drops peers idle >60s). Dumb room relay: first connection = P1 (host), second = P2 (guest), extras get `{t:'full'}`; an optional 4-digit room code (`?code=`) locks the room so only a peer with the matching code can join (backward compatible — no code = open room). Run: `node lan-server.js [port]` (default 8000); prints LAN URLs. |
